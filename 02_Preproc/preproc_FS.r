...
#################################################################################
#                    PREPROCESSING FOR RAW FIXED SENSOR DATA                    #
...
stagrid = stagrid[which(stagrid$pm10 >= 4),] # Apply the repreatability criterion caluculated from 01_Stat/repeatability.r
stagrid = stagrid[which(stagrid$pm25 >= 2),] # Apply the repreatability criterion caluculated from 01_Stat/repeatability.r
...
#  Adjustment for day to day differences in background concentrations
for (iday in 1:length(day_list)){
    estim_day <- day_list[iday]

# Estimation of the backgroud concentration from each sensor observations = the median concentration within a moving 15-min window
start_date <- paste0(estim_year,"-",estim_month,"-",estim_day," 00:00:00") 
...
# Subset ref data depending on iday
data_pol_ref1_sub <- data_pol_ref1[which(data_pol_ref1$date >= times_start & data_pol_ref1$date <= times_end),] # Subset dataframe
data_pol_ref1_sub <- subset(data_pol_ref1_sub, select=-c(date))
...
for (nsens in 1:length(nsensd)){
    ...
    data_sens_len <- length(data_sens[,1]) # Sub dataset length
    lon_sens <- data_sens$lon[1]
    lat_sens <- data_sens$lat[1]

    # Look for the closest station
    diff1 = (abs(lat_sens-lat1)+abs(lon_sens-lon1)); diff2 = (abs(lat_sens-lat2)+abs(lon_sens-lon2)); diff3 = (abs(lat_sens-lat3)+abs(lon_sens-lon3));
    diff4 = (abs(lat_sens-lat4)+abs(lon_sens-lon4)); diff5 = (abs(lat_sens-lat5)+abs(lon_sens-lon5));
    diff_all = c(diff1,diff2,diff3,diff4,diff5)
    idx = which(diff_all == min(diff_all))

    code_pol = as.integer(code_sta_pol[idx])

    tmp_ref_pol <- data_pol_ref1_sub[which(data_pol_ref1_sub$code_station == code_pol),] # Select the corresponding measurement in the reference dataframe

    # A) Estimate daily background concentration from reference stations over the sampling period = daily mean
    BG_REF_pol <- mean(tmp_ref_pol$pol) # daily mean
    ...
    data_runf5 <- c(); 
    data_sens2 <- c()
    idx_runs <- matrix(NA,20,6)
    idx_runs2 <- matrix(NA,20,6)
    NEW_RUN=FALSE

    # Loop over sensor data
    for (dd in 1:data_sens_len){
        count=count+1
        ...
        if ( dd < data_sens_len){ 
            data_run[count,2:8]=data_sens[dd,]
        ...
        if (delta_t >= maxdelta_t){maxdelta_t=delta_t}
            ...
            #### Smoothing data for monitoring run with different time resolutions ####
            ...
            print("!! DO SMOOTHING !!")
                ...
                for (nres in 1:length(res_time)){
                    ...
                    #### Underwrite function: capture baseline trends in the time-series data (ith percentile over the moving window) ####
                    ...
                    for (mm in 1:length(data_runf[,1])){
                         ...
                         options(digits=4)
                         x_bg_pol[mm] = as.numeric(quantile(pol_conc, c(nq))) # ith percentile
                         Cf_pol <- (Ci_pol - x_bg_pol[mm]) + BG_REF_pol
                         if (Cf_pol<0 | is.na(Cf_pol)){Cf_pol = NA}
                         data_runf$pol_bgcorr[mm] <- Cf_pol
                     }        

                    # Plot
                    #png(filename=paste(dirout,"FS",nsensd[nsens],"_",estim_year,estim_month,estim_day,"_run_bgdata_perc_",nq,'_res',res_time[nres],"ss.png",sep=""),width=1200, height=400)     
                    ...
                    #abline(h=BG_REF_pol, v = 0, col ="gray60",lwd=2)
                    #r <- as.POSIXct(round(range(data_runf$datetime), "hours"))
                    #axis.POSIXct(1, at = seq(r[1], r[2], by = "hours" ), format = "%H:%M",cex.axis=1.2)
                    #grid (NULL,NULL, lty = 6) 
                    #title(main=paste('FS',nsensd[nsens],"  ",estim_year,"-",estim_month,"-",estim_day,"  run",run,' time res=',res_time[nres],"mins",sep=""),cex.main=1.6,xlab="Time",ylab=paste0(pol," (ug/m3)"),cex.lab=1.6)
                    ...
                    #Plot
                    #print("DO PLOT CORRECTION")
                    #png(filename=paste(dirout,"FS",nsensd[nsens],"_",estim_year,estim_month,estim_day,"_run_bgdata_perc_",nq,"_",pol,".png",sep=""),width=1200, height=400)  
                    ...
                    #axis.POSIXct(1, at = seq(r[1], r[2], by = "hours" ), format = "%H:%M",cex.axis=1.2)
                    #grid (NULL,NULL, lty = 6) 
                    #title(main=paste('FS',nsensd[nsens],"  ",estim_year,"-",estim_month,"-",estim_day,"  run",run,sep=""),cex.main=1.6,xlab="Time",ylab=paste0(pol," (ug/m3)"),cex.lab=1.6)
                    #legend("topright",legend=c("raw data", "corr data","data BG","ref BG"), col=c("black", "blue","red","gray60"),horiz = TRUE, cex=1.6,lty=c(1,1,2,1),lwd=2, bty="n")
                    #dev.off()
...
if (SMOOTHING==TRUE){
    ...
    save(data_preproc1,file=paste0(dirout2,"data_preproc_FS_bgdata_",nq,"_res1.Rda"))
    data_preproc2 = subset(data_preproc2, select=-c(index_t))
    save(data_preproc2,file=paste0(dirout2,"data_preproc_FS_bgdata_",nq,"_res2.Rda"))
    data_preproc3 = subset(data_preproc3, select=-c(index_t))
    save(data_preproc3,file=paste0(dirout2,"data_preproc_FS_bgdata_",nq,"_res3.Rda"))
    data_preproc4 = subset(data_preproc4, select=-c(index_t))
    save(data_preproc4,file=paste0(dirout2,"data_preproc_FS_bg_data_",nq,"_res4.Rda"))
    data_preproc5 = subset(data_preproc5, select=-c(index_t))
    save(data_preproc5,file=paste0(dirout2,"data_preproc_FS_bg_data_",nq,"_res5.Rda"))
}else{
    data_preproc = subset(data_preproc, select=-c(index_t))
    save(data_preproc,file=(paste0(dirout2,"data_preproc_FS_bgdata_",nq,"_",estim_year,estim_month,estim_day,".Rda")))
}
}
} # If there is data
} # Loop over days
